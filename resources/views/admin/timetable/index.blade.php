...
<div class="animate-fadeInUp">
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-end">
                <a href="{{ route('admin.timetable.create') }}" class="btn btn-sm btn-primary">
        ...
        <div class="card-body">
            <form method="GET" class="mb-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        <a href="{{ route('admin.timetable.index') }}" class="btn btn-secondary w-100">Reset</a>
                    </div>
                </div>
            </form>
            ...
            @if($selectedClass && $selectedSection)
                <div class="mb-3">
                    {{-- <a href="{{ route('admin.timetable.edit-grid', [$selectedClass, $selectedSection]) }}" class="btn btn-warning">
                        <i class="fas fa-edit me-1"></i> Edit Grid (All Days)
                    </a> --}}
                    {{-- <a href="{{ route('admin.timetable.export', [$selectedClass, $selectedSection]) }}" class="btn btn-info">
                                       ...
                                       @php
                                        ...
                                        <td class="align-middle {{ $isToday ? 'today-col' : '' }} {{ ($isToday && $isCurrentSlot && !$slot->is_break) ? 'current-slot' : '' }}">
                                            ...
                                            @if($entry && !$slot->is_break)
                                                ...
                                                <div class="mt-1">
                                                    <a href="{{ route('admin.timetable.edit', $entry) }}" class="btn btn-xs btn-outline-primary px-2 py-1">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form action="{{ route('admin.timetable.destroy', $entry) }}" method="POST" class="d-inline">
                                                        @csrf @method('DELETE')
